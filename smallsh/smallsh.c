...
char *expandPID(char *inputString)
{
    ...
    //Copy values from original string to temp
    ...
    for (int i = 0; i < numChars-1; i++) {
        ...
        if (inputString[i] == '$' && inputString[i+1] == '$') {
            ...
            temp[i] = '\0';                                 //Remove first '$'
            strcat(temp, pid);                              //Add pid to temp

            j += (int)strlen(pid) - 1;                      //Move j to account for PID length
            i++;                                            //Skip second '$'
        }
    ...
    //Copy temp to original input string, free memory, and return
    sprintf(inputString, "%s", temp);
